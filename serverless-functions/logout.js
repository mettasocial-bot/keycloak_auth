...
function logoutAdmin(access_token) {
  fetch(
    `https://auth.meraklis.in/auth/admin/realms/meraklis-platform/users/${
...
export async function index(event) {
  ...
  try {
    ...
    let res = await fetch(
      `https://auth.meraklis.in/auth/realms/meraklis-platform/protocol/openid-connect/token`,
      {
        headers: {
    ...
    res = await fetch(
      `https://auth.meraklis.in/auth/admin/realms/meraklis-platform/users/${userId}/logout`,
      {
        headers: {
          Authorization: `Bearer ${access_token}`,
          "Content-Type": "application/json",
